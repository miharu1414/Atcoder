...
int main(){
	...
	/*仕様のメモ
		...
		mean_rank[i]：iグループの順位を格納（小さいほうから）
		group_member[i][0]：iグループの人数
		group_member[i][j]：iグループのj番目の人の学籍番号 j>=1
		pos_gr:今何個目のグループにいるか（小さい順にアクセスするときに使う）
	...
	*/
	...
	for(roop = 1; roop <= (n + k - 1)/k; roop++){//何巡必要か計算　切り上げ
		for(pos_gr = 0; pos_gr < k&& left_man>0; pos_gr++){
			group_member[group_rank[pos_gr]][roop] = number_gakuseki[now];
			left_man--;
			now++;
			group_member[group_rank[pos_gr]][0]++;
		}
		if(left_man>0){
			...
			for(m = 0;m<k-1;m++){
				for( temp_p = mean_group[m+1], temp_num = group_rank[m+1],i = m;i>=0;i--){
					if(temp_p >= mean_group[i])break; //降順，大きい順だから
